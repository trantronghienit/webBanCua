<?php 
	class Myclass
	{
		public function category_dequi($data, $parentid)
		{
			echo "<ul>";
			foreach ($data as $key => $m) 
			{
				...
				{
					echo '<li><a href="index.php?option=sanpham&cat='.$m["link"].'">'.$m["name"].'</a>';
					$this -> category_dequi($data, $id);
					echo '</li>';
				}	
			}
			echo '</ul>';
		}
		function str_alias($alias)
		{
